...
export async function shoppingCartRoutes(fastify: FastifyInstance) {
  ...
  async function assertCartOwner(
    cartId: string,
    request: Parameters<typeof authenticateCustomer>[0],
  ) {
    const result = await query(
      "SELECT 1 FROM shopping_carts WHERE id = $1 AND customer_id = $2 AND status = 'ACTIVE'",
      [cartId, customerId(request)],
    );
  ...
  fastify.put("/shopping-cart/items/:itemId", async (request, reply) => {
    ...
    try {
      const owner = await query(
        "SELECT 1 FROM cart_items ci JOIN shopping_carts sc ON sc.id = ci.cart_id WHERE ci.id = $1 AND sc.customer_id = $2 AND sc.status = 'ACTIVE'",
        [itemId, customerId(request)],
      );
      if (!owner.rowCount)
  ...
  fastify.put("/shopping-cart/:cartId/status", async (_request, reply) => {
    return reply.status(403).send({
      error: "Cart lifecycle status is controlled by the checkout service",
    });
  });
  ...
  fastify.post("/shopping-cart/cleanup", async (_request, reply) => {
    return reply
      .status(403)
      .send({ error: "Cart cleanup is an internal operation" });
  });
}
