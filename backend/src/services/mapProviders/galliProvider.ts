...
export class GalliProvider extends BaseMapProvider {
  constructor() {
    super(
      "Galli",
      process.env.GALLI_API_KEY || "",
      process.env.GALLI_BASE_URL || "https://api.galli.com/api/v1",
    );
  }
  ...
  async geocode(request: GeocodeRequest): Promise<GeocodeResponse> {
    ...
    // In production, call Galli's geocoding API
  ...
  async reverseGeocode(
    ...
    // In production, call Galli's reverse geocoding API
    // Mock response for now
    return {
  ...
  async autocomplete(
    ...
    // In production, call Galli's autocomplete API
    // Mock response for now
    return {
      predictions: [
        {
  ...
  async getRoute(request: RouteRequest): Promise<RouteResponse> {
    ...
    // In production, call Galli's routing API
    // Mock response for now
    return {
      distance: 1000,
      duration: 300,
  ...
  async getDistanceMatrix(request: DistanceRequest): Promise<DistanceResponse> {
    ...
    // In production, call Galli's distance matrix API
    // Mock response for now
